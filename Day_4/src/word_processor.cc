...
WordProcessor::WordProcessor(const std::string& word_to_find) {
    std::copy(word_to_find.begin(), word_to_find.end(), std::back_inserter(word_to_find_));
}
...
uint64_t WordProcessor::FindXMAS() {
    num_of_matches_ = 0;
